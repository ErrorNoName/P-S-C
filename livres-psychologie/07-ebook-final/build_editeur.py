...
"""
    css = asset(0, "css/editeur.css") + "?v=float3"
    scripts = (
        f'<script src="{asset(0, "js/lame.min.js")}?v=float3"></script>\n'
        f'<script src="{asset(0, "js/editeur-format.js")}?v=float3"></script>\n'
        f'<script src="{asset(0, "js/editeur.js")}?v=float3"></script>'
    )
    html = page_shell(
        "Cahier",
        body,
